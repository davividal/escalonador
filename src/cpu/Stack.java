...
public class Stack {
	private static final Integer MAX_PROCESSES = 5; //20;
	private static final Integer MAX_PROCESSES_ROUND = 1; //5;
	...
	public void run() {
		...
		for (Process process: this.queue) {
			roundProcesses.put(process, process.getStatus());
		}
		
		for (Process process: this.blocked) {
			roundProcesses.put(process, Process.Status.BLOCKED);
		}
